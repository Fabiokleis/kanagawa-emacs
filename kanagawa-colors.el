...
(defmacro kanagawa-define-variant-colors (variant &rest body)
  `(defvar ,(intern (format "kanagawa-%s-colors" (symbol-name variant)))
     (let ,(mapcar #'(lambda (p) `(,(car p) ,(cdr p))) kanagawa-color-palette-alist)
       ,@body)))

(kanagawa-define-variant-colors
 wave
 `((fg . ,fuji-white)
   (fg-dim . ,old-white)
   ...
   (syn-parameter . ,oni-violet2)
   (syn-fun . ,crystal-blue)
   (syn-statement . ,oni-violet)
   (syn-keyword . ,oni-violet)
   (syn-operator . ,boat-yellow2)
   (syn-perproc . ,wave-red)
   (syn-perproc . ,wave-aqua2)
   (syn-regex . ,boat-yellow2)
